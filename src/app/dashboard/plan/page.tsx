            ...
            <div className="min-h-screen flex flex-col">
                <div className="bg-card rounded-none shadow-lg overflow-hidden border-0 flex-1 flex flex-col">
                    ...
                    <div className="p-3 sm:p-4 md:p-6 flex-1 flex flex-col">
                        <Tabs value={`day-${currentDay + 1}`} onValueChange={(val) => setCurrentDay(parseInt(val.split('-')[1]) - 1)} className="w-full flex-1 flex flex-col">
                            ...
                            {plan.map((dayPlan, dayIndex) => (
                                <TabsContent key={dayPlan.day} value={`day-${dayPlan.day}`} className="mt-0 flex-1 min-h-0">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row">
                                                        {meal.imageUrl ? (
                                                            <Image 
                                                                ...
                                                                width={120}
                                                                height={120}
                                                                className="w-full h-32 sm:w-28 sm:h-auto flex-shrink-0 object-cover"
                                                                unoptimized
                                                            />
                                                        ) : (
                                                            <div className="w-full h-32 sm:w-28 sm:h-auto flex-shrink-0 bg-muted flex items-center justify-center">
                                                        ...
                                                        <div className="flex-1 p-3 sm:p-4 min-w-0">
                                                            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                                                                <div className="flex-1 min-w-0">
                                                                    <h3 className="font-bold text-base text-foreground leading-tight">{meal.meal}</h3>
                                                                    <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                                                        <span className="px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded-full font-medium">{meal.calories}cal</span>
                                                                        <span className="px-2 py-0.5 bg-purple-100 text-purple-800 text-xs rounded-full">{meal.mealTime}</span>
                                                                    </div>
                                                                    <p className="text-muted-foreground text-xs leading-relaxed mt-2 line-clamp-2">{meal.description}</p>
                                                                </div>
                                                                <Button 
                                                                    onClick={() => toggleMealCompletion(dayIndex, mealIndex)} 
                                                                    variant={meal.completed ? 'default' : 'secondary'} 
                                                                    size="sm"
                                                                    className="flex-shrink-0 text-xs px-3 py-1.5 h-auto mt-2 sm:mt-0"
                                                                >
